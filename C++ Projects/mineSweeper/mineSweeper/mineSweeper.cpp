...
void MineSweeper::initializeField(std::vector<std::vector<char>>& field, char simbol)
{
...
void MineSweeper::initializeMines(std::vector<std::vector<char>>& field)
{
	...
	for (int i = 0; i < m_minesCount; i++)
	{
		while (true)
		{
			...
			if (field[m_randRow][m_randCol] != '1')
			{
				field[m_randRow][m_randCol] = '1';
				break;
			}
		}
	}
...
void MineSweeper::shouldIncrementScore(int row, int col)
{
	if (m_hiddenField[row][col] == '0')
	{
		m_score++;
	}
}

void MineSweeper::shouldIncrementTries(int row, int col)
{
	if (m_hiddenField[row][col] == '0')
	{
		m_triesProgression++;
...
std::string MineSweeper::checkInput(int row, int col) const
{
	if (row > m_rowSize)
	{
		return "You're sweeping way past the minefield! You lost?";
	}
	if (row < 0)
	{
		return "You're sweeping before the minefield! Careful to not get lost!";
	}
	if (col > m_colSize)
	{
		return "You're sweeping way to the right! Are you trying to find Narnia?";
	}
	if (col < 0)
	{
		return "You're sweeping too far to the left! Are you searching for your lost sanity?";
	}

	if (m_visibleField[row][col] == '0')
	{
		return "You've swept this spot again! Are you practicing your sweeping skills safely?";
	}
...
std::vector<std::vector<char>>& MineSweeper::getHiddenField()
{
	return m_hiddenField;
}

std::vector<std::vector<char>>& MineSweeper::getVisibleField()
{
	return m_visibleField;
}


void MineSweeper::drawScreen(const std::vector<std::vector<char>>& field) const
{
	system("cls");
	std::cout << "   ";
	...
	std::cout << "   SCORE: " << m_score << " | TRIES: " << m_tries << std::endl;
	std::cout << std::endl;
}
